...
const looksLikeDataGovLandingPage = (url) => /data\.gov\.in\/resource\/[^?]+/i.test(url) && !/api\.data\.gov\.in/i.test(url);
...
router.get('/prices', async (req, res) => {
    try {
        ...
        const response = await axios_1.default.get(base, axiosConfig);
