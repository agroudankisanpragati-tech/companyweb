...
router.post('/detect-disease', async (req: Request, res: Response) => {
  try {
...
router.post('/recommendations', async (req: Request, res: Response) => {
  try {
    ...
    // scoring
    const scoreCrop = (c: any) => {
      let score = c.profitPotential || 50;
      if (soilType && typeof soilType === 'string') {
        const st = soilType.toLowerCase();
        if (st.includes('sandy') && c.waterRequirement === 'Low') score += 15;
        if (st.includes('loamy')) score += 10;
        if (st.includes('clay') && c.waterRequirement === 'High') score += 10;
      }
      if (waterAvailability === 'low' && c.waterRequirement === 'Low') score += 20;
      if (waterAvailability === 'high' && c.waterRequirement === 'High') score += 15;
      if (avgRainfall && avgRainfall > 800 && c.waterRequirement === 'High') score += 10;
      if (farmSize && farmSize > 10 && c.crop === 'Cotton') score += 10; // larger farms prefer cash crops
      return score;
    };
    ...
    const saved = await Promise.all(
      ranked.map(async (r) => {
        const rec = new CropRecommendation({
